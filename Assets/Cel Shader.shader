...
// Sourced from: https://en.wikibooks.org/wiki/Cg_Programming/Unity/Toon_Shading
// Edited by Zak
Shader "Cg shader for toon shading" {
   Properties {
      _Color ("Diffuse Color", Color) = (1,1,1,1) 
      _UnlitColor ("Unlit Diffuse Color", Color) = (0.2,0.2,0.2,1) 
      _DiffuseThreshold ("Threshold for Diffuse Colors", Range(0,5)) 
         = 0.1 
      _OutlineColor ("Outline Color", Color) = (0,0,0,1)
      _LitOutlineThickness ("Lit Outline Thickness", Range(0,1)) = 0.25
   ...
   SubShader {
      Pass {      
         ...
         float4 frag(vertexOutput input) : COLOR
         {
            ...
            // default: unlit 
            float3 fragmentColor = (_Color.rgb+_UnlitColor.rgb)/3; 
            ...
            // second diffuse illumination part.
            if (attenuation 
               * max(0.0, dot(normalDirection, lightDirection)) 
               >= _DiffuseThreshold*5)
            {
               fragmentColor = _LightColor0.rgb *2 * _Color.rgb; 
            }

            // third diffuse illumination part. may add more.
            if (attenuation 
               * max(0.0, dot(normalDirection, lightDirection)) 
               >= _DiffuseThreshold*8)
            {
               fragmentColor = _LightColor0.rgb *2.5 * _Color.rgb; 
            }
            ...
            if (dot(viewDirection, normalDirection) 
               ...
               max(0.0, dot(normalDirection, lightDirection))))
            {
               fragmentColor = (_Color.rgb+_UnlitColor.rgb)/3; //_LightColor0.rgb * _OutlineColor.rgb; 
            }
